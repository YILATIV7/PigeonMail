...
public class Pigeon implements Comparable<Pigeon>, Cloneable, Visualizable {
    ...
    private double x, y;
    private double[] moveVector = {0, 0};  // посилальний тип, для якого потрібно застосувати глибинне копіювання
    ...
    public void setDegrees(int degrees) {
        moveVector[0] = Math.cos(Math.PI * degrees / 180);
        moveVector[1] = Math.sin(Math.PI * degrees / 180);
    }
    ...
    public void move(double dt) {
        int speed = 100;
        x += dt * speed * moveVector[0];
        y += dt * speed * moveVector[1];
        invalidatePosition();
    }
    ...
    public String toString() {
        return "Pigeon{" +
                "x=" + x +
                ", y=" + y +
                ", moveVector=" + Arrays.toString(moveVector) +
                ", moveType=" + moveType +
                ", name='" + name + '\'' +
                '}';
    }
    ...
    protected Object clone() throws CloneNotSupportedException {
        ...
        pigeon.moveVector = new double[]{moveVector[0], moveVector[1]};
        return pigeon;
    }
}
